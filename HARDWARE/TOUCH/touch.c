...
void TP_Drow_Touch_Point(u16 x,u16 y,u16 color)
{
	...
	LCD_DrawPoint(x+1,y-1);
	LCD_DrawPoint(x-1,y-1);
	LCD_Draw_Circle(x,y,6,POINT_COLOR);//画中心圈
}	  
//画一个大点(2*2的点)		   
//x,y:坐标
//color:颜色
...
{	  
	POINT_COLOR=RED;
	LCD_ShowString(40,160,lcddev.width,lcddev.height,16,"x1:",0x01);
 	LCD_ShowString(40+80,160,lcddev.width,lcddev.height,16,"y1:",0x01);
 	LCD_ShowString(40,180,lcddev.width,lcddev.height,16,"x2:",0x01);
 	LCD_ShowString(40+80,180,lcddev.width,lcddev.height,16,"y2:",0x01);
	LCD_ShowString(40,200,lcddev.width,lcddev.height,16,"x3:",0x01);
 	LCD_ShowString(40+80,200,lcddev.width,lcddev.height,16,"y3:",0x01);
	LCD_ShowString(40,220,lcddev.width,lcddev.height,16,"x4:",0x01);
 	LCD_ShowString(40+80,220,lcddev.width,lcddev.height,16,"y4:",0x01);  
 	LCD_ShowString(40,240,lcddev.width,lcddev.height,16,"fac is:",0x01);     
...
{								 
	...
	LCD_Clear(WHITE);//清屏 	   
	POINT_COLOR=BLACK;
	LCD_ShowString(40,40,160,100,16,(u8*)TP_REMIND_MSG_TBL,0x01);//显示提示信息
				...
				case 4:	 //全部四个点已经得到
					...
					if(abs(tp_dev.xfac)>2||abs(tp_dev.yfac)>2)//触屏和预设的相反了.
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20,WHITE);	//清除点4
   	 					TP_Drow_Touch_Point(20,20,RED);								//画点1
						LCD_ShowString(40,26,lcddev.width,lcddev.height,16,"TP Need readjust!",0x01);
					...
					POINT_COLOR=BLUE;
					LCD_Clear(WHITE);//清屏
					LCD_ShowString(35,110,lcddev.width,lcddev.height,16,"Touch Screen Adjust OK!",0x01);//校正完成
					delay_ms(1000);
					TP_Save_Adjdata();  
 					LCD_Clear(WHITE);//清屏   
					return;//校正完成				 
  ...
  GPIO_InitTypeDef  GPIO_InitStructure;	
    ...
    GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化
	  ...
	  GPIO_Init(GPIOF, &GPIO_InitStructure);//初始化			
		...
		if(KEY0==1&&TP_Get_Adjdata())return 0;//开机后按住KEY0键进行校准
		else			   //未校准?
		{ 										    
			LCD_Clear(WHITE);//清屏
			TP_Adjust();  	//屏幕校准 
