...
u16 POINT_COLOR = BLACK; //初始化画笔颜色为黑色
u16 BACK_COLOR = WHITE; //背景颜色为白色

_lcd_dev lcddev;//管理变量




//初始化lcddev的参数
//lcd_dir；显示方式 1竖屏 0横屏
void lcddev_init(u8 lcd_dir)
{
	lcddev.width = 320;
	lcddev.height = 480;
	lcddev.id = 0x5310;
	lcddev.dir = lcd_dir;//竖屏
	lcddev.setxcmd = 0x2A;
	lcddev.setycmd = 0x2B;
	lcddev.wramcmd = 0x2C;
}
...
void LCD_Init(u8 dir)
{
	...
	readWriteTiming.FSMC_AddressSetupTime = 0X03;	 //地址建立时间（ADDSET）为3个HCLK 1/168M=6ns*3=18ns	
	readWriteTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（ADDHLD）模式A未用到	
	readWriteTiming.FSMC_DataSetupTime = 9;			//数据保存时间为10个HCLK=6*10=60ns
	...
	writeTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（A模式未用到）		
...
void LCD_Scan_Dir(u8 dir)
{
	if(dir == 1)//竖屏的时候
	{
		LCD_WriteReg(0x36,0x00); //从左到右，从上到下
	}
	else if(dir == 0)//横屏的时候
	{
		LCD_WriteReg(0x36,0x60);//从上到下，从右到左
	}
...
void LCD_Display_Dir(u8 dir)
{
	...
	else if(dir == 0)//横屏
	{
		...
		lcddev.height = 320;
		lcddev.dir = 0;//表示横屏显示
		//为何不改变 写x命令和写y命令的原因是 将采用改变扫描方式的方法
	}
	LCD_Scan_Dir(dir);//设置自动扫描方式
}
...
void LCD_Clear(u16 color)
{
	...
	LCD->LCD_REG = lcddev.wramcmd;//准备写GRAM
...
//叠加方式：不覆盖背景，非叠加方式：用背景颜色覆盖原来的背景色
...
//num:为字库中的横坐标
//size：大小
//mode：是否采用叠加模式 1：采用叠加模式 0：不采用叠加模式
void LCD_ShowChineseChar(u16 x,u16 y,u8 num,u8 size, u8 mode)
{
	...
	{
		temp = LinXingYu[num][t1];       //LinXingYu 为已提取的字库，根据自己需要提取
		for(t=0;t<8;t++)
		{
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
			else if(mode==0)LCD_Fast_DrawPoint(x,y,BACK_COLOR);
